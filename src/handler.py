...
def putRecordsToKinesisStream(streamName, records, client, attemptsMade, maxAttempts):
    ...
    if failedRecords:
        if attemptsMade + 1 < maxAttempts:
            print(
                'Some records failed while calling PutRecords to Kinesis stream, retrying. %s' % (errMsg))
...
def lambda_handler(event, context):
    ...
    if recordListsToReingest:
        ...
        client = boto3.client(
            'kinesis' if isSas else 'firehose', region_name=region)
        maxBatchSize = 500
        flattenedList = [
            r for sublist in recordListsToReingest for r in sublist]
        for i in range(0, len(flattenedList), maxBatchSize):
            ...
            print('Reingested %d/%d' %
                  (recordsReingestedSoFar, len(flattenedList)))
